...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        depth, alpha = render_pkg["rendered_depth"], render_pkg["rendered_alpha"]
        ...
        depth = depth[:, crop_h: temp_h - crop_h, crop_w: temp_w - crop_w]
        alpha = alpha[:, crop_h: temp_h - crop_h, crop_w: temp_w - crop_w]
        ...
        if iteration % 200 == 0:
            ...
            gt_mask_show = ((gt_mask.float()).clamp(0,1).unsqueeze(-1).repeat(1, 1, 3)*255).detach().cpu().numpy().astype(np.uint8)
            row0 = np.concatenate([img_show, gt_img_show, gt_mask_show], axis=1)
            image_to_show = row0
