...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy,
        ...
        if self.preload_img:
            ...
            depth_name, normal_name, mask_name = "depths", "normals", "masks"
                
            base_path = self.image_path.split('/')[:-2]
            depth_path = base_path[0]
            for path in base_path[1:]:
                depth_path = os.path.join(depth_path, path)
                
            original_depth = cv2.imread(os.path.join('/' + depth_path, depth_name, self.image_name.split('.')[0] + '0001.exr'), cv2.IMREAD_UNCHANGED)[..., 0:1]
            self.original_depth = torch.from_numpy(original_depth).float().permute(2, 0, 1).to(self.data_device)
            
            # normal_path = base_path[0]
            # for path in base_path[1:]:
            #     normal_path = os.path.join(normal_path, path)
            # self.original_normal = torch.load(os.path.join('/' + normal_path, normal_name, self.image_name.split('.')[0] + '.pt'), weights_only=False).permute(2, 0, 1).to(self.data_device)
            
            # mask_path = base_path[0]
            # for path in base_path[1:]:
            #     mask_path = os.path.join(mask_path, path)
            # mask = Image.open(os.path.join('/' + mask_path, mask_name, self.image_name.split('.')[0] + '.png'))
            # self.original_mask = torch.from_numpy(np.array(mask))[..., :1].permute(2, 0, 1).to(self.data_device) / 255.0
    ...
    def get_depth(self):
        return self.original_depth.cuda()

    # def get_normal(self):
    #     return self.original_normal.cuda()
    
    # def get_mask(self):
    #     return self.original_mask.cuda()
