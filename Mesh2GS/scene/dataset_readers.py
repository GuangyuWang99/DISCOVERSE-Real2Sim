...
from scene.gaussian_model import BasicPointCloud, BasicMesh
...
class SceneInfo(NamedTuple):
    ...
    manifold: Union[BasicMesh, BasicPointCloud]
    train_cameras: list
    test_cameras: list
...
def readColmapSceneInfo(path, images, eval, llffhold=1000):
    try:
    ...
    # hold_out_list = list(range(199, 267)) + list(range(316, 439)) + list(range(532, 646)) + list(range(689, 798))
    # cam_infos = [cam_infos[i] for i in range(len(cam_infos)) if i not in hold_out_list]
    # cam_infos = cam_infos[:30]
    ...
    mesh = BasicMesh(verts=verts, faces=faces, normals=normals)

    scene_info = SceneInfo(manifold=mesh,
