...
from utils.graphics_utils import BasicPointCloud, BasicMesh
...
class GaussianModel:
    ...
    def create_from_manifold(self, mesh: BasicMesh, spatial_lr_scale: float):
        self.spatial_lr_scale = spatial_lr_scale
        
        face_verts = mesh.verts[mesh.faces]   # [F, 3, 3]
        central_points = (face_verts[:, 0] + face_verts[:, 1] + face_verts[:, 2]) / 3.

        central2verts = face_verts - central_points[:, None, :]
        dist = torch.sqrt(torch.from_numpy((central2verts ** 2).sum(axis=-1).mean(axis=-1)).float())
        scales = dist[..., None].repeat(1, 3)

        tagents = face_verts[:, 0] - central_points
        tagents = tagents / np.linalg.norm(tagents, axis=-1, keepdims=True)
        mask = (mesh.normals ** 2).sum(axis=-1) == 0.
        mesh.normals[mask] = np.array([1.0, 0., 0.])
        orthogs = np.cross(mesh.normals, tagents, axis=-1)
        tagents = torch.from_numpy(tagents).float()
        normals = torch.from_numpy(mesh.normals).float()
        orthogs = torch.from_numpy(orthogs).float()
        rot_matrices = torch.zeros((central_points.shape[0], 3, 3))
        rot_matrices[:, :, 0] = tagents
        rot_matrices[:, :, 1] = orthogs
        rot_matrices[:, :, 2] = normals
        rots = matrix_to_quaternion(rot_matrices)
        
        num_pts = central_points.shape[0]
        fused_point_cloud = torch.tensor(central_points).float().cuda()
